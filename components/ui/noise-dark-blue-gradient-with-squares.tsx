...
interface GridProps {
  ...
  borderColor: string;
  vignette?: boolean;
  gridOffsetRef: React.RefObject<Offset>;
  className?: string;
}
...
const MovingGrid: React.FC<GridProps> = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const offset = gridOffsetRef.current ?? { x: 0, y: 0 };
...
export default function NoiseDarkBlueGradientWithSquares({
  ...
  return (
    <div aria-hidden="true" className={cn("absolute inset-0 z-50 bg-neutral-950", className)}>
      {/* soft spotlight tone */}
